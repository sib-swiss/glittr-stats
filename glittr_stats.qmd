...
Create a file named `.env.R` and add your GitHub PAT (named `pat` ) and google api key (named `google_api_key`) in there, e.g.:

```{r, eval=FALSE}
# this is an example, store it as .env.R:
pat <- "ghp_aRSRESCTZII20Lklser3H"
google_api_key <- "AjKSLE5SklxuRsxwPP8s0"
```
